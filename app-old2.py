...
=====================================================
Formules calquées exactement sur le fichier Excel :
  - Feuil1 : sans éthane, sans inflation
  - Feuil2 : avec éthane, sans inflation
  - Feuil3 : avec inflation sur OPEX

Formule clé du Cash Flow Net après Impôt :
  Année 0  → NCF = -CAPEX  (pas d'impôt)
  Année i  → NCF = (Revenus - OPEX) × (1 - Taux_Impôt)

VAN calculée comme Excel : NPV(taux, CF0..CFn) = Σ CFi / (1+taux)^(i+1)
...
st.caption("Formules identiques au fichier Excel (Feuil1 / Feuil2 / Feuil3)")
...
with st.sidebar:
    ...
    avec_inflation = st.checkbox("Appliquer l'inflation sur l'OPEX", value=False,
                                  help="Coché = Feuil3 (OPEX augmente chaque année). Décoché = Feuil1/Feuil2 (OPEX fixe).")
    inclure_ethane = st.checkbox("Inclure le revenu Éthane", value=True,
                                  help="Coché = Feuil2 (avec éthane). Décoché = Feuil1 (sans éthane).")
...
# -----------------------------------------------------------------------
# CALCULS  (identiques aux formules Excel)
# -----------------------------------------------------------------------

# Revenus annuels (constants)
# Formule Excel : = débit × prix × 24 × jours / 1 000 000
...
for annee in range(0, duree + 1):
    if annee == 0:
        ...
        c = 0.0
        # OPEX : avec ou sans inflation
        if avec_inflation:
            o = opex_base * (1 + taux_inflation) ** (annee - 1)
        else:
            o = opex_base
        rg = rev_gaz
        rp = rev_propane
        ...
        # CF Net après Impôt = (Revenus - OPEX) × (1 - Taux_Impôt)
...
# -----------------------------------------------------------------------
# VAN  — réplique exacte de =NPV() d'Excel
# Excel : NPV(taux, CF0:CFn) = Σ CFi / (1+r)^(i+1)   i = 0..n
# -----------------------------------------------------------------------
cf_list = df["CF Net après Impôt (MM$)"].tolist()

def van_excel(taux, cfs):
    return sum(cf / (1 + taux) ** (i + 1) for i, cf in enumerate(cfs))

van = van_excel(taux_actualisation, cf_list)


# -----------------------------------------------------------------------
# TRI  — réplique de =IRR() d'Excel
# Cherche r tel que Σ CF(i) / (1+r)^i = 0
# -----------------------------------------------------------------------
def calcul_tri(cfs, precision=1e-8, max_iter=500):
    def npv_standard(taux):
        return sum(cf / (1 + taux) ** i for i, cf in enumerate(cfs))

    lo = -0.50
    if npv_standard(lo) < 0:
        return None
    hi = None
...
def calcul_payback(cumul):
    for i in range(1, len(cumul)):
        if cumul[i] >= 0 and cumul[i - 1] < 0:
            ...
            ans = int(total_mois // 12)
            mois = int(round(total_mois % 12))
            return ans, mois
    return None, None
...
# -----------------------------------------------------------------------
if inclure_ethane and not avec_inflation:
    mode = "Feuil2 (avec éthane, sans inflation)"
elif not inclure_ethane and not avec_inflation:
    mode = "Feuil1 (sans éthane, sans inflation)"
elif avec_inflation:
    mode = "Feuil3 (avec inflation sur OPEX)"
else:
    mode = ""

st.info(f"📋 Mode correspondant Excel : **{mode}**")
...
# VÉRIFICATION
# -----------------------------------------------------------------------
with st.expander("✅ Vérification : valeurs attendues du fichier Excel"):
    st.markdown("""
    | Indicateur | Feuil1 (sans éthane) | Feuil2 (avec éthane) |
    |---|---|---|
    | **VAN (MM$)** | 9.929 | 10.959 |
    | **TRI (%)** | 25.73% | 27.26% |
    | **Payback** | ~4 ans | 4 ans |
    | **Cumul an 20** | 37.80 | 40.46 |
    | **CF net/an après impôt** | 2.340 | 2.473 |
    """)
    st.caption("Compare les indicateurs ci-dessus avec ce tableau pour vérifier.")
...
tab1, tab2, tab3, tab4 = st.tabs([
    "📈 Cash-flow cumulé",
    "💰 CF Net après Impôt (barres)",
    "🔄 Comparaison avec/sans inflation",
    "📋 Tableau détaillé"
])
...
with tab3:
    # Scénario inverse pour comparer
    cumul_compare = []
    total_cf = 0
    for annee in range(0, duree + 1):
        if annee == 0:
...
with st.expander("📐 Détail des formules utilisées (identiques à Excel)"):
    st.markdown(f"""
    **Revenus annuels** (constants chaque année) :
    - Gaz = {debit_gaz} × {prix_gaz} × 24 × {jours} / 1 000 000 = **{rev_gaz:.4f}** MM$
    - Propane = {prod_propane} × {prix_propane} × 24 × {jours} / 1 000 000 = **{rev_propane:.4f}** MM$
    - Butane = {prod_butane} × {prix_butane} × 24 × {jours} / 1 000 000 = **{rev_butane:.4f}** MM$
    - Éthane = {prod_ethane} × {prix_ethane} × 24 × {jours} / 1 000 000 = **{rev_ethane:.4f}** MM$
    - **Total = {rev_total:.4f} MM$/an**

    **Cash Flow Net après Impôt** :
    - Année 0 : NCF = −CAPEX = **−{capex}** MM$ *(pas d'impôt)*
    - Année i : NCF = (Revenus − OPEX) × (1 − {taux_impot:.0%})
    - Ex. Année 1 : ({rev_total:.4f} − {opex_base:.2f}) × {1-taux_impot:.2f} = **{(rev_total - opex_base) * (1 - taux_impot):.4f}** MM$

    **VAN** (fonction NPV d'Excel, décalée d'1 période) :
    - VAN = Σ CF(i) / (1 + {taux_actualisation:.0%})^(i+1)   pour i = 0 à {duree}

    **TRI** (fonction IRR d'Excel) :
    - Taux r tel que Σ CF(i) / (1+r)^i = 0

    **Payback** :
    - Année où le cumul du CF net après impôt passe de négatif à positif
    """)

st.divider()
st.caption("Calculateur calqué sur les formules Excel du projet Sabria — "
           "NCF après impôt, VAN (NPV Excel), TRI (IRR), Payback.")
